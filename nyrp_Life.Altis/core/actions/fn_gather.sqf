...
switch(true) do {
	case (_zone in ["apple_1","apple_2","apple_3","apple_4"]): {_gather = ["apple",2 + (round random 8)];};
	case (_zone in ["peaches_1","peaches_2","peaches_3","peaches_4"]): {_gather = ["peach",2 + (round random 8)];};
	case (_zone in ["heroin_1"]): {_gather = ["heroin_unprocessed",1 + (round random 3)];};
	case (_zone in ["cocaine_1"]): {_gather = ["cocaine_unprocessed",1 + (round random 2)];};
	case (_zone in ["weed_1"]): {_gather = ["cannabis",1 + (round random 4)];};
	default {""};
};
//gather check??
