...
_unit spawn
{
	...
	if((life_request_cooldown - time) > 0) then {
		waitUntil {_Timer ctrlSetText format[localize "Auf einen Notarzt warten: %1",[(life_request_cooldown - time),"MM:SS.MS"] call BIS_fnc_secondsToString];
		round(life_request_cooldown - time) <= 0 OR isNull _this};
	};
