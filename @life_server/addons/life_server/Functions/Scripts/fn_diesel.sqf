...
//1.00 = voll = 100% annahme 65 Liter fasst Tank von Offroad w?hren: 1.00 / 65L = 0.0154L
...
if(_action) then {
	...
	if(life_atmbank < _cost) exitWith {hint format["Du hast nicht genuegend Geld auf der Bank! Du benoetigst %1",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"];};
	life_atmbank = life_atmbank - _cost;
	...
	if (fuel _vehicle > 0.99) then {
		5 cutText ["","PLAIN"];
		hint format ["Dein Fahrzeug %1 wurde f?r %2? vollgetankt. Wir wuenschen eine gute Weiterfahrt" , _vehicleName,[_cost] call life_fnc_numberText];
