...
compileFinal "
	...
	if(side player != west) exitWith {hint ""You are not an Cop!"";};
	private[""_msg"",""_from""];
	_msg = ctrlText 3003;
	if(_msg == """") exitWith {hint ""You must enter a message to send!"";};

	[[_msg,name player,7],""TON_fnc_clientMessage"",true,false] call life_fnc_MP;
	[] call life_fnc_cellphone;
	hint format[""Polizeidurchsage Sent To All: %1"",_msg];
";
...
TON_fnc_cell_serverMessageall =compileFinal "
	if(isServer) exitWith {};
	if((call life_adminlevel) < 1) exitWith {hint ""You are not an Admin!"";};
	private[""_msg"",""_from""];
	_msg = ctrlText 3003;
		...
		case 7 :
	        {
	            ...
	            [""TextMessage"",[""Durchsage der Polizei empfangen""]] call bis_fnc_showNotification;
	            systemChat _message;
	            if((call life_adminlevel) > 0) then {systemChat _admin;};
	        };
	};
